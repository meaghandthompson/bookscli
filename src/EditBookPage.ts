...
class EditBookPage {
    public async render(rl: readline.Interface, books: Book[]) {
        const idEntered = await askQuestion(rl, "What is the ID of the book to edit?");
        ...
        let editedBook: Book = { Id: id, ISBN: isbn, Title: title, Author: author };
        filteredBooks.push(editedBook);
